...
minishift: local-dev/minishift/minishift
	$(info starting minishift with name $(MINISHIFT_PROFILE))
	MINISHIFT_ENABLE_EXPERIMENTAL=y ./local-dev/minishift/minishift --profile $(MINISHIFT_PROFILE) start --cpus $(MINISHIFT_CPUS) --memory $(MINISHIFT_MEMORY) --disk-size $(MINISHIFT_DISK_SIZE) --vm-driver virtualbox --openshift-version="v3.9.0" --extra-clusterup-flags "--service-catalog"
ifeq ($(ARCH), Darwin)
	...
	oc --context="default/$$(./local-dev/minishift/minishift --profile $(MINISHIFT_PROFILE) ip | sed 's/\./-/g'):8443/system:admin" adm policy add-cluster-role-to-user cluster-admin system:anonymous; \
	oc --context="default/$$(./local-dev/minishift/minishift --profile $(MINISHIFT_PROFILE) ip | sed 's/\./-/g'):8443/system:admin" adm policy add-cluster-role-to-user cluster-admin developer;
	...
	sleep 60
	eval $$(./local-dev/minishift/minishift --profile $(MINISHIFT_PROFILE) oc-env); \
	for i in {10..30}; do oc --context="default/$$(./local-dev/minishift/minishift --profile $(MINISHIFT_PROFILE) ip | sed 's/\./-/g'):8443/system:admin" patch pv pv00$${i} -p '{"spec":{"storageClassName":"bulk"}}'; done;
